...
export class NgcRangeComponent extends Utilities implements OnInit, OnChanges {
  ...
  constructor(private el: ElementRef) {
    // call utilies class from here
    super();
  }

  @Input('currencySymbol')
  set setCurrencySymbol(value: string) {
    if (value) {
  ...
  public initializeSlider() {
    try {
      ...
      // console.log('SliderInit nativeElement ', this.el.nativeElement)
      // console.log('SliderInit element ', this.sliderWidth)
      this.resetModel();
  ...
  private updateCurrentValue(arrayValue: number[], privateChange: boolean = false) {
    ...
    if (!privateChange) {
      this.rangeChange.emit((this.multiRange) ? this.currentValues : [this.currentValues[0]]);
    }
  }
